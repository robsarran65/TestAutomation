...
from framework.ai_engine import ai_explain_failure
...
def run_test_from_excel(file):
    ...
    # Load data-driven rows if available
    data_rows = []
    try:
        data_rows = load_data_file("generated_data.xlsx")
    except:
        pass
    ...
    chart_path = f"logs/{filename}_chart.png"
...
def generate_html_report(all_tests, filename, chart_path):
    os.makedirs("logs", exist_ok=True)
    path = f"logs/{filename}_report.html"
...
def generate_pdf_report(all_tests, filename):
    os.makedirs("logs", exist_ok=True)
    path = f"logs/{filename}_report.pdf"
