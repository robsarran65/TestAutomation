"""
Supporting agents that analyse a run rather than execute one.

    LocatorRepairAgent      - proposes replacement XPaths when one breaks,
                              ranked by a confidence score (the "self-healing"
                              half of the framework)
    DataValidatorAgent      - checks test data for missing fields, empty
                              values, duplicates and schema mismatches
    PerformanceAnalyzerAgent - derives timing metrics from a finished run and
                              flags steps slower than a threshold

None of these touch a browser or the network, which is why they are the
easiest agents to unit-test.
"""
...
class LocatorRepairAgent(Agent):
    ...
    def __init__(self):
        """Create the agent. Stateless -- safe to reuse across runs."""
